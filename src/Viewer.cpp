...
//查看器的构造函数
Viewer::Viewer(FrameDrawer *pFrameDrawer, Group *pGroup, const string& strSettingPath):
    mpFrameDrawer(pFrameDrawer), mpGroup(pGroup),
    mbFinishRequested(false), mbFinished(true), mbStopped(false), mbStopRequested(false)
{
    ...
    //从配置文件中获取图像的长宽参数
    mImageWidth = fSettings["width"];
    mImageHeight = fSettings["height"];
    ...
    cv::FileStorage fS("/home/simulator/LIC/hrs_vo/camera_models/Viewer.yaml", cv::FileStorage::READ);
    //读取视角
...
void Viewer::Run()
{
    ...
    pangolin::CreateWindowAndBind("HRS_VO: Map Viewer", 1024, 768);
    ...
    cv::namedWindow("HRS_VO: Current Frame");
    ...
    while(1)
    {
        ...
        cv::imshow("HRS_VO: Current Frame", im);
        cv::waitKey(mT);
