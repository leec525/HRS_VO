 ...
 *  1.1 只更新关键帧的地图点深度,新的关键帧到来时，中断updateSeed()
 *  1.2 setExistingFeatures()设置包含feature的栅格为占据状态，这时的feature只来自与相邻关键帧匹配得到，所以不需要通过深度滤波器再优化了
 *  1.3 使用feature_detector_->detect()检测新的feature，然后加入到seeds_中
 * 2. 使用updateSeeds()遍历每一个seed
 *  2.1 首先检测该seed在当前帧中是否可见
 *  2.2 接着使用findEpipolarMatchDirect()处理这个seed
 *    2.2.1 对于边缘特征（Feature::EDGELET），如果把梯度仿射过来后，梯度的方向与极线方向的夹角大于45度，
 *         就认为沿着极线找，图块像素也不会变化很大，就不搜索了，直接返回false
 *    2.2.2 沿着极线方向搜索，在当前帧上找到与ref_ftr对应的图像块最匹配的图像块，接着使用align2D()进一步优化，
 *         得到更为精准的px_cur_，而后就可以使用seed的对应的ref_ftr与px_cur_进行三角化，得到3D point，也就得到了待估计的深度（depth）
 *  2.3 上一步得到深度，接下来使用updateSeed()得到深度的协方差，这里要参考论文:《 Video-based, Real-Time Multi View Stereo》
 *  2.4 如果协方差小于阈值，就认为收敛了，它就不再是种子点，而是candidate点，使用回调函数，加入到候选点队列中
 *  2.5 如果第2步匹配成功，则得到px_cur_，如果当前帧是关键帧的话，就将matcher_.px_cur_所在的栅格设置为占据状态
 */
